...
@section('title', 'Chọn Môn học - ' . $classroom->name)
    ...
    <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; background: #ffffff;">
        <div>
            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                <a href="{{ route('departments.index', ['department_id' => $classroom->department_id]) }}" style="text-decoration: none; color: #94a3b8; display: flex; align-items: center;" title="Quay lại">
                    <i data-lucide="arrow-left" style="width: 16px;"></i>
                </a>
                <h3 style="font-size: 1.15rem; color: var(--text-title); margin: 0; font-weight: 700;">Danh sách Môn học - Lớp {{ $classroom->name }}</h3>
            </div>
            <p style="color: var(--text-body); font-size: var(--fs-xs);">Chọn môn học cụ thể để xem danh sách sinh viên đăng ký học phần này.</p>
        </div>
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 0.4rem 0.75rem; border-radius: 6px; display: flex; align-items: center; gap: 6px;">
            <i data-lucide="book-open" style="width: 14px; color: var(--brand-primary);"></i>
            <span style="font-size: 11px; font-weight: 700; color: #475569;">{{ $subjects->count() }} Môn học</span>
        </div>
    </div>
        ...
        <table class="data-table" style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background: #f8fafc;">
                    ...
                    <th style="padding: 0.75rem 1rem; text-align: center; font-size: var(--fs-xs); color: #475569; border-bottom: 2px solid #e2e8f0; width: 160px;">TÍN CHỈ</th>
            ...
            <tbody id="subject-table-body">
                ...
                <tr class="table-row" style="border-bottom: 1px solid #f1f5f9;">
                    ...
                    <td style="padding: 0.85rem 1rem; text-align: right;">
                        <a href="{{ route('departments.index', ['department_id' => $classroom->department_id, 'classroom_id' => $classroom->id, 'subject_id' => $sub->id]) }}" class="btn-icon" title="Xem sinh viên">
