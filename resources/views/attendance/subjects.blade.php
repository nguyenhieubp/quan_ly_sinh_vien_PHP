...
@section('title', 'Điểm danh - Chọn Môn')
    ...
    <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; background: #ffffff;">
        <div>
            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                <a href="{{ route('attendance.index', ['department_id' => $classroom->department_id]) }}" style="text-decoration: none; color: #94a3b8; display: flex; align-items: center;" title="Quay lại">
                    <i data-lucide="arrow-left" style="width: 16px;"></i>
                </a>
                <h3 style="font-size: 1.15rem; color: var(--text-title); margin: 0; font-weight: 700;">Hệ thống Điểm danh - Lớp {{ $classroom->name }}</h3>
            </div>
            <p style="color: var(--text-body); font-size: var(--fs-xs);">Bước 3: Chọn Môn học để tiến hành điểm danh chi tiết.</p>
        </div>
    </div>
        ...
        <table class="data-table" style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background: #f8fafc;">
                    <th style="padding: 0.75rem 1rem; text-align: left; font-size: var(--fs-xs); color: #475569; border-bottom: 2px solid #e2e8f0; width: 140px;">MÃ MÔN</th>
                    <th style="padding: 0.75rem 1rem; text-align: left; font-size: var(--fs-xs); color: #475569; border-bottom: 2px solid #e2e8f0;">TÊN MÔN HỌC / HỌC PHẦN</th>
                    <th style="padding: 0.75rem 1rem; text-align: center; font-size: var(--fs-xs); color: #475569; border-bottom: 2px solid #e2e8f0; width: 120px;">TÍN CHỈ</th>
                ...
                <tr style="background: #ffffff; border-bottom: 1px solid #e2e8f0;">
                    ...
                    <td style="padding: 0.5rem 1rem;">
                        <input type="text" class="col-filter" data-col="1" placeholder="Lọc tên môn..." 
            ...
            <tbody id="subject-table-body">
                ...
                <tr class="table-row" style="border-bottom: 1px solid #f1f5f9;">
                    <td style="padding: 1rem; font-weight: 700; color: var(--brand-primary); font-family: monospace; font-size: var(--fs-sm);">{{ $sub->code }}</td>
                    <td style="padding: 1rem; font-weight: 600; color: var(--text-title); font-size: var(--fs-sm);">{{ $sub->name }}</td>
                    <td style="padding: 1rem; text-align: center;">
                        <span style="font-weight: 700; color: #64748b; font-size: var(--fs-xs);">{{ $sub->credits }} TC</span>
                    </td>
                    <td style="padding: 1rem; text-align: right;">
                        <a href="{{ route('attendance.create', ['classroom_id' => $classroom->id, 'subject_id' => $sub->id]) }}" class="btn-icon" title="Điểm danh">
